...
namespace FastTrundle
{
    ...
    internal static class Trundle
    {
        ...
        public static void Game_OnGameLoad(EventArgs args)
        {
            ...
            Obj_AI_Base.OnDoCast += ObjAIBase_OnDoCast;
        }
        ...
        private static void OnUpdate(EventArgs args)
        {
            ...
            if (Player.IsDead || Player.IsRecalling() || MenuGUI.IsChatOpen || MenuGUI.IsShopOpen) return;
                ...
                if (IsActive("FastTrundle.LaneClear.Q.Lasthit"))
                {
                    if (minion.Health <= QDamage(minion)
                        && (minion.Health > Player.GetAutoAttackDamage(minion) ||
                            (!Player.IsWindingUp && !Orbwalking.CanAttack()))) // don't overkill with Q unless we need AA reset to get it
                    {
                        spells[Spells.Q].Cast();
